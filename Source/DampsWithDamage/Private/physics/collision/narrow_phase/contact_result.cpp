...
void ContactPoint::getOrthoUnits(pe::Vector3 normal, pe::Vector3 &tangent1, pe::Vector3 &tangent2) {
    ...
    if (PE_ABS(normal.z) > PE_R(0.7071)) {
        ...
        const pe::Real k = PE_R(1.0) / std::sqrt(a);
        tangent1.x = 0;
...
void ContactResult::setObjects(pe_physics_object::RigidBody* object_a,
                               pe_physics_object::RigidBody* object_b) {
    _object_a = object_a;
    _object_b = object_b;
...
void ContactResult::addContactPoint(const pe::Vector3& world_normal,
    ...
    if (cp_idx >= 0) {
        ...
        if (!found_empty && idx >= 0) {
            // replace only if the new depth is deeper or equal (but with smaller x value)
            if (depth < max_dist - PE_EPS || (depth < max_dist + PE_EPS && point.x < _points[idx].getWorldPos().x))
...
void ContactResult::sortContactPoints() {
    std::sort(_points, _points + PE_CONTACT_CACHE_SIZE,
        ...
        // Do not use depth for sorting to maintain stability
        // Only make sure invalid points are at the end
