...
void BroadPhaseSweepAndPrune::calcCollisionPairs(pe::Array<pe_physics_object::RigidBody*> objects,
    ...
    std::sort(objects.begin(), objects.end(),
              [this](const pe_physics_object::RigidBody* cb1, const pe_physics_object::RigidBody* cb2) {
    ...
    for (int i = 0; i < PE_I(objects.size()); i++) {
        ...
        pe_physics_object::RigidBody* cb1 = objects[i];
        ...
        for (int j = i + 1; j < PE_I(objects.size()); j++) {
            pe_physics_object::RigidBody* cb2 = objects[j];
    ...
    pe::Vector3 v = s2 - s.mult(s) / PE_R(objects.size());
    _target_axis = 0;
