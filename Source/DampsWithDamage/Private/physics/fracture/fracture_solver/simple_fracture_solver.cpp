...
COMMON_FORCE_INLINE bool isPointWithinHalfImpactRadius(const pe::Array<FractureSource>& sources, pe::Real threshold, const pe::Vector3& pos) {
    for (auto& fs : sources) {
        const pe::Real half_impact_radius = (PE_SQRT(PE_MAX(fs.intensity.x * fs.intensity.x + fs.intensity.y * fs.intensity.y + fs.intensity.z * fs.intensity.z, PE_R(1e-15)))) / threshold * PE_R(0.8);
        pe::Vector3 diff = pos - fs.position;
        if (diff.dot(diff) <= half_impact_radius * half_impact_radius) {
            return true;
        }
    }
    return false;
...
void SimpleFractureSolver::solve(const pe::Array<FractureSource>& sources) {
    ...
    pe::Mesh mesh;
    if (shape->getType() == pe_physics_shape::ShapeType::ST_ConvexMesh) {
        mesh = static_cast<pe_physics_shape::ConvexMeshShape *>(shape)->getMesh();
    } else if (shape->getType() == pe_physics_shape::ShapeType::ST_Box) {
        mesh = static_cast<pe_physics_shape::BoxShape *>(shape)->getMesh();
    } else return;
    ...
    for (int i = 0; i < PE_I(fragments.size()); i++) {
        if (!fragments[i].empty()) {
            auto filename = "fragment-" + std::to_string(i);
            auto rb = addMesh(fragments[i], world_trans, filename);
            if (!isPointWithinHalfImpactRadius(sources, threshold, world_trans * points[i])) {
                rb->setKinematic(true);
            }
            pe::Vector3 vel = rb->getLinearVelocity();
            vel += forces[i] / PE_MAX(rb->getMass(), 1e-15);
            const pe::Transform& old_trans = rb->getTransform();
            const pe::Transform new_trans(old_trans.getBasis(), old_trans.getOrigin() + vel * PE_R(0.01));
            rb->setTransform(new_trans);
            if (_fracturable_object->getCollisionShape()->localIsInside(_fracturable_object->getTransform().inverseTransform(rb->getTransform().getOrigin()))) {
                rb->setKinematic(true);
            }
            rb->setLinearVelocity(vel * PE_R(0.1));
