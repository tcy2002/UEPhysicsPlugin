...
pe::Matrix3 from_two_vectors(const pe::Vector3& a, const pe::Vector3& b) {
    ...
    c_hat[2][0] = -c.y;
    c_hat[2][1] = c.x;
    return pe::Matrix3::identity() + c_hat + c_hat * c_hat * (1 - c_) / PE_MAX(s * s, 1e-15);
}
...
void calc_tet_bounding_sphere(const pe::Vector3& v1, const pe::Vector3& v2,
    ...
    const pe::Real k1 = v1v3.dot(v1v3m - v1v2m) / (u1 < 0 ? PE_MIN(u1, -1e-15) : PE_MAX(u1, 1e-15));
    const pe::Real k2 = v1v4.dot(v1v4m - v1v3m) / (u2 < 0 ? PE_MIN(u2, -1e-15) : PE_MAX(u2, 1e-15));

    const auto p1 = v1v2m + v1v2n * k1;
    const auto p2 = v1v3m + v1v3n * k2;

    const pe::Real k3 = v1v4.dot(p2 - p1) / (u3 < 0 ? PE_MIN(u3, -1e-15) : PE_MAX(u3, 1e-15));
...
bool are_points_collinear(const pe::Vector3& p1, const pe::Vector3& p2, const pe::Vector3& p3) {
    return PE_APPROX_EQUAL((p1 - p2).cross(p3 - p2).norm2(), 1e-8);
}
