...
#   endif
    ...
    // calculate unique edges: each edge is represented by two vertices and its 2 adjacent faces
    _unique_edges.clear();
    pe::Vector3HashList vert_map(PE_UI(_mesh.vertices.size()));
    pe::Map<pe::KV<uint32_t, uint32_t>, pe::KV<int, int>> edge_map;
    for (int fi = 0; fi < PE_I(_mesh.faces.size()); fi++) {
        auto& f = _mesh.faces[fi];
        for (int vi = 0; vi < PE_I(f.indices.size()); vi++) {
            auto v0 = f.indices[vi];
            auto v1 = f.indices[(vi + 1) % f.indices.size()];
            ...
            if (edge_map.find({id0, id1}) == edge_map.end()) {
                edge_map[{id0, id1}] = {fi, -1}; // face1_id must be on the left
                _unique_edges.push_back({_mesh.vertices[v0].position, _mesh.vertices[v1].position, id0, id1});
            } else {
                edge_map[{id0, id1}].second = fi;
            }
        }
    }
    for (auto& ue : _unique_edges) {
        auto& fs = edge_map[{ue.face1_id, ue.face2_id}];
        ue.face1_id = fs.first;
        ue.face2_id = fs.second;
        if (fs.first == -1 || fs.second == -1) {
            PE_LOG_ERROR << "Invalid unique edges detected." << PE_ENDL;
        }
    }
...
pe::Vector3 ConvexMeshShape::getLocalSupportPoint(const pe::Vector3 &dir) const {
    pe::Real max_dot = PE_REAL_MIN;
    pe::Vector3 support_point = pe::Vector3::zeros();
    for (auto &p: _mesh.vertices) {
        pe::Real d = p.position.dot(dir);
        if (d > max_dot) {
            max_dot = d;
            support_point = p.position;
        }
    }
