...
BoxShape::BoxShape(const pe::Vector3 &size): _size(size), _half_size(size / 2) {
    ...
    _unique_edges = PE_BOX_UNIQUE_EDGES;
...
pe::Vector3 BoxShape::getLocalSupportPoint(const pe::Vector3 &dir) const {
    pe::Vector3 support;
    support.x = dir.x >= 0 ? _half_size.x : -_half_size.x;
    support.y = dir.y >= 0 ? _half_size.y : -_half_size.y;
    support.z = dir.z >= 0 ? _half_size.z : -_half_size.z;
    return support;
}
